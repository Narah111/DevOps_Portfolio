import { useNavigate } from 'react-router-dom'
import Navbar from '../components/Navbar' // 🔥 viktigt
import './LandingPage.css'


function LandingPage() {
  const navigate = useNavigate()

  return (
    <div className="landing">
      {/* 🔥 Använd din riktiga navbar */}
      <Navbar />

      <main className="landing-hero">
        <div className="hero-badge">Built on AWS · Serverless · IaC</div>
        <h1>
          Track bugs.<br />
          <span>Ship faster.</span>
        </h1>

        <p className="hero-subtitle">
          A lightweight bug tracker built with React, AWS Lambda, API Gateway,
          DynamoDB and Cognito. Fully serverless, fully yours.
        </p>

        <div className="hero-buttons">
          <button
            onClick={() => navigate('/register')}
            className="btn-primary btn-large"
          >
            Get started for free
          </button>

          <button
            onClick={() => navigate('/login')}
            className="btn-ghost btn-large"
          >
            Sign in
          </button>
        </div>
      </main>

      <section className="landing-features">
        <h2 className="section-title">Key Features</h2>

        <div className="features-container">
          <div className="feature-card">
            <h3>Serverless</h3>
            <p>
              Powered by AWS Lambda and API Gateway. No servers to manage,
              scales automatically.
            </p>
          </div>

          <div className="feature-card">
            <h3>Secure</h3>
            <p>
              Authentication via AWS Cognito. Your bugs are private and only
              visible to you.
            </p>
          </div>

          <div className="feature-card">
            <h3>CI/CD</h3>
            <p>
              Deployed automatically via GitHub Actions every time code is
              pushed to main.
            </p>
          </div>

          <div className="feature-card">
            <h3>Infrastructure as Code</h3>
            <p>
              All AWS resources are defined and managed with OpenTofu.
              Reproducible and versioned.
            </p>
          </div>
        </div>
      </section>

      <footer className="landing-footer">
        <p>Built with React · AWS Lambda · DynamoDB · Cognito · OpenTofu</p>
      </footer>
    </div>
  )
}
